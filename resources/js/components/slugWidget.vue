  ...
  export default {
    ...
    methods: {
      adjustWidth: function (event) {
        ...
        if (key === "Escape") {
          ...
          document.getElementById('slug-editor').style.width = Match.ceil(ctx.measureText(val).width+25)+"px";
        }
      },
      editSlug: function() {
      ...
      saveSlug: function() {
        // to later run ajax to see if new slug is unique
    ...
    watch: {
      title: _.debounce(function(){
          ...
          // to later run ajax to see if slug is unique
        }, 500)
    }
  }
</script>
